...
public class AudioCodec {
    ...
    public void prepare() {
        ...
        if (encodeType.equals(MediaFormat.MIMETYPE_AUDIO_AAC)) {
            mMediaEncoder = getAACMediaEncode();//AAC编码器
        } else if (encodeType.equals(MediaFormat.MIMETYPE_AUDIO_MPEG)) {
            mMediaEncoder = getMPEGMediaEncode();//mp3编码器
        }
        ...
        mMediaEncoder.start();
        encodeInputBuffers = mMediaEncoder.getInputBuffers();
        encodeOutputBuffers = mMediaEncoder.getOutputBuffers();
...
//                    format.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 200 * 1024);
                    ...
                    mMediaDecoder = MediaCodec.createDecoderByType(mime);//创建Decode解码器
                    mMediaDecoder.configure(format, null, null, 0);
                    break;
                }
            }
        } catch (IOException e) {
        ...
        decodeInputBuffers = mMediaDecoder.getInputBuffers();//MediaCodec在此ByteBuffer[]中获取输入数据
        decodeOutputBuffers = mMediaDecoder.getOutputBuffers();//MediaCodec将解码后的数据放到此ByteBuffer[]中 我们可以直接在这里面得到PCM数据
     ...
     * @return
     */
    private MediaCodec getAACMediaEncode() {
        try {
     ...
     * 初始化MPEG编码器
     */
    private MediaCodec getMPEGMediaEncode() {
        return null;
    }
    ...
    private void srcAudioFormatToPCM() {
        for (int i = 0; i < decodeInputBuffers.length - 1; i++) {
            int inputIndex = mMediaDecoder.dequeueInputBuffer(-1);//获取可用的inputBuffer -1代表一直等待，0表示不等待 建议-1,避免丢帧
            if (inputIndex < 0) {
                codeOver = true;
                return;
            }
            ...
            if (sampleSize < 0) {//小于0 代表所有数据已读取完成
                ...
                mMediaDecoder.queueInputBuffer(inputIndex, 0, sampleSize, 0, 0);//通知MediaDecode解码刚刚传入的数据
        ...
        int outputIndex = mMediaDecoder.dequeueOutputBuffer(decodeBufferInfo, 10000);
...
//        showLog("decodeOutIndex:" + outputIndex);
        ...
        while (outputIndex >= 0) {//每次解码完成的数据不一定能一次吐出 所以用while循环，保证解码器吐出所有数据
            ...
            mMediaDecoder.releaseOutputBuffer(outputIndex, false);//此操作一定要做，不然MediaCodec用完所有的Buffer后 将不能向外输出数据
            outputIndex = mMediaDecoder.dequeueOutputBuffer(decodeBufferInfo, 10000);//再次获取数据，如果没有数据输出则outputIndex=-1 循环结束
        }
...
//        showLog("doEncode");
        for (int i = 0; i < encodeInputBuffers.length - 1; i++) {
            ...
            inputIndex = mMediaEncoder.dequeueInputBuffer(-1);//同解码器
            inputBuffer = encodeInputBuffers[inputIndex];//同解码器
            inputBuffer.clear();//同解码器
            inputBuffer.limit(chunkPCM.length);
            inputBuffer.put(chunkPCM);//PCM数据填充给inputBuffer
            mMediaEncoder.queueInputBuffer(inputIndex, 0, chunkPCM.length, 0, 0);//通知编码器 编码
        }

        outputIndex = mMediaEncoder.dequeueOutputBuffer(encodeBufferInfo, 10000);//同解码器
...
//                showLog("outPacketSize:" + outPacketSize + " encodeOutBufferRemain:" + outputBuffer.remaining());
            ...
            outputIndex = mMediaEncoder.dequeueOutputBuffer(encodeBufferInfo, 10000);
